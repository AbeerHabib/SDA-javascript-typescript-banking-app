...
class Customer {
    ...
    getBalance() {
        return this.transactions.reduce((totalBalance, currentAmount) => totalBalance + currentAmount.amount, 0);
    }
}
class Branch {
    constructor(name) {
...
class Bank {
    ...
    addCustomerTransaction(branch, customerId, amount) {
        if (this.checkBranch(branch)) {
            const customer = branch.customers.find(customer => customer.id == customerId);
            if (customer) {
                if (amount < 0) {
                    console.log('Negative values are not accepted.');
                    return false;
